...
def scanner_smb_ghost_silent(ip,port):
    ...
    if version != 0x0311:
        print(f"SMB version {hex(version)} foi encontrado que não é vulnerável!")
        return False
    elif context != 2:
        print(
            f"Servidor respondeu com contexto {hex(context)} o que indica que o destino pode não ter a compactação SMB habilitada e, portanto, não é vulnerável!")
        return False
    else:
        print(
            f"SMB versão {hex(version)} com contexto{hex(context)} foi encontrado o que indica que o SMBv3.1.1 está sendo usado e a compactação SMB está habilitada, sendo portanto vulnerável para CVE-2020-0796!")
        return True
